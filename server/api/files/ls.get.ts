...
export default defineEventHandler(async (event) => {
  ...
  if (typeof name !== "string") {
    return {
      error: {
        code: FileLsErrorCode.INVALID_PARAM,
        message: 'Expect the "name" query param to be string',
      },
    };
  }
  if (!event.context.auth) {
    return {
      error: {
        code: FileLsErrorCode.NOT_ENOUGH_PRIVILEGE,
        message: "Should be logged in as a user with enough privilege",
      },
    };
  }
  ...
  if (!filepath.isValid()) {
    return {
      error: {
        code: FileLsErrorCode.INVALID_PARAM,
        message: 'Expect the "name" query param to be valid path',
      },
    };
  }
  try {
    const {
      permission_bits: filePermissionBits,
      owner_id: fileOwnerId,
      group_id: fileGroupId,
      file_type: fileType,
      created_at: createdAt,
      updated_at: updatedAt,
    } = await db.selectExactlyOne("files", {
    ...
    if (
      !canAccess(
        {
          userId: event.context.auth.userId as number,
          groupId: event.context.auth.groupId as number,
        },
        {
    ...
    const files = await db.select("files", {
      name: db.conditions.and(
        db.conditions.like(`${filepath.toString()}/%`),
    ...
    return {
      error: {
        code: FileLsErrorCode.FILE_NOT_FOUND,
