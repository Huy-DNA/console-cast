...
export default defineEventHandler(async (event) => {
  ...
  if (
    ...
    return { error: { message: 'Invalid body. Expected "src" and "dest" to be strings and permission_bits to be a bit string.' } };
  }

  const src = VirtualPath.createUnchecked(body.src);
  if (!src.isValid()) {
    setResponseStatus(event, 400);
    return { error: { message: 'Expect the "src" param to be valid path' } };
  }

  const dest = VirtualPath.createUnchecked(body.dest);
  if (!dest.isValid()) {
    setResponseStatus(event, 400);
    return { error: { message: 'Expect the "dest" param to be valid path' } };
  }

  if (src.isAncestor(dest)) {
    setResponseStatus(event, 400);
    return { error: { message: 'Cannot move a folder to it descendant' } };
  }
  ...
  const result = await db.serializable(dbPool, async (dbClient) => {
    ...
    if (
      ...
      return { error: { message: 'Should be logged in as a user with enough privilege' }, statusCode: 403 };
    }
